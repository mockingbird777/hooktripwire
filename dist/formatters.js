...
export function formatTerminal(result, color = false) {
    ...
    for (const finding of result.findings) {
        ...
        lines.push(`${colored}  ${visibleControls(finding.location.path)}:${finding.location.line}:${finding.location.column}`);
        lines.push(`  ${visibleControls(finding.title)} — ${visibleControls(finding.message)}`);
        lines.push(`  Evidence: ${visibleControls(finding.evidence)}`);
        lines.push(`  Fix: ${visibleControls(finding.remediation)}`);
