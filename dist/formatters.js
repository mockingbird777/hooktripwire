...
import { escapeHtml, escapeMarkdown, safeJson, toPosix, visibleControls } from "./utils.js";
...
function findingIds(path, result) {
    const byFingerprint = new Map(result.findings.map((finding) => [finding.fingerprint, finding.ruleId]));
    return [...new Set(path.findingFingerprints.map((fingerprint) => byFingerprint.get(fingerprint)).filter((id) => id !== undefined))].sort();
}
export function formatTerminal(result, color = false) {
    ...
    for (const finding of result.findings) {
        ...
        if (finding.suppressed === true)
            lines.push(`  Suppressed: ${singleLine(finding.suppressionReason ?? "policy")}`);
        lines.push("");
    }
    if (result.hookPaths !== undefined) {
        lines.push(`Hook execution paths (${result.hookPaths.length}):`);
        if (result.hookPaths.length === 0)
            lines.push("  No statically provable local hook paths discovered.");
        for (const [index, hookPath] of result.hookPaths.entries()) {
            ...
            if (ids.length > 0)
                lines.push(`     Findings: ${ids.join(", ")}`);
            if (hookPath.incomplete !== undefined)
                lines.push(`     Incomplete: ${hookPath.incomplete}`);
        }
        lines.push("");
    }
...
export function formatMarkdown(result) {
    ...
    if (result.hookPaths !== undefined) {
        ...
        for (const hookPath of result.hookPaths) {
            ...
            const ids = findingIds(hookPath, result);
            if (ids.length > 0)
                lines.push(`  - Findings: ${ids.map(markdownCode).join(", ")}`);
            if (hookPath.incomplete !== undefined)
                lines.push(`  - Incomplete: ${markdownCode(hookPath.incomplete)}`);
        }
    }
