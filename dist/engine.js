...
function findingsFor(input, policy, baseline) {
    return runRules(input, policy).map((raw) => {
        const meta = metadata.get(raw.ruleId);
        if (meta === undefined)
            throw new Error(`Internal error: metadata missing for ${raw.ruleId}`);
...
export function scanText(content, displayPath = "input.sh", policyInput = {}) {
...
export async function audit(request) {
    ...
    if (request.maxHookDepth !== undefined)
        validateMaxHookDepth(request.maxHookDepth);
    ...
    for (const item of [...discovered.skipped, ...(mapped?.skipped ?? [])])
