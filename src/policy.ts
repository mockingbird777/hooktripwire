...
function severityMap(value: unknown): Record<string, Severity> {
  ...
  for (const [id, raw] of Object.entries(value)) {
    if (!RULE_IDS.has(id)) throw new Error(`Unknown rule in severity policy: ${id}`);
...
export function normalizePolicy(input: Partial<Policy> = {}): Policy {
  for (const field of Object.keys(input)) {
    if (!POLICY_FIELDS.has(field)) throw new Error(`Unknown policy field: ${field}`);
  }
  ...
  for (const id of disabledRules) {
    if (!RULE_IDS.has(id)) throw new Error(`Unknown disabled rule: ${id}`);
  }
  ...
  if (trustedActions.some((action) => action.trim().length === 0)) throw new Error("Policy trustedActions entries cannot be empty");
