...
function lineRule(meta: RuleMetadata, expression: RegExp, message?: string): Rule {
  return {
    meta,
    detect(context) {
...
function isShellComment(context: LineContext): boolean {
...
const rules: readonly Rule[] = [
  {
    meta: destructive,
    detect(context) {
      if (isShellComment(context)) return undefined;
  ...
  },
  {
    ...
    detect(context) {
      ...
      if (!/(?:\$(?:\{|\()?(?:[A-Z0-9_]*(?:TOKEN|SECRET|PASSWORD|PASSWD|API_KEY|PRIVATE_KEY|CREDENTIAL)[A-Z0-9_]*)|~?\/?(?:\.ssh|\.aws|\.gnupg|\.config\/gh)|\/etc\/(?:shadow|passwd))/i.test(command)) return undefined;
      const host = hostFrom(command);
      if (host !== undefined && hostAllowed(host, context.policy)) return undefined;
  ...
  },
  {
    ...
    detect(context) {
      ...
      if (direct !== null) return hit(injection, context, direct);
  ...
  },
  {
    ...
    detect(context) {
      ...
      if (trace !== null) return hit(secretOutput, context, trace, "Shell tracing can copy later credentials into logs.", "medium");
  ...
  },
  {
    ...
    detect(context) {
      if (isShellComment(context) || !/\b(?:curl|wget)\b/i.test(context.window)) return undefined;
      if (/\b(?:curl|wget)\b[^\n]{0,500}(?:\n[^\n]{0,500}){0,2}?\|\s*(?:sudo\s+)?(?:sh|bash|zsh|fish|node|python\d*|ruby|perl)\b/i.test(context.window)) return undefined;
      if (/\b(?:sha256sum|shasum\s+-a\s+256|cosign\s+verify|gpg\s+--verify|openssl\s+dgst)\b/i.test(context.window)) return undefined;
  ...
  },
  {
    ...
    detect(context) {
      ...
      if (!explicitMutable && hasVersion) return undefined;
      if (["node", "python", "ruby", "ubuntu", "alpine"].includes(image) || image.includes("/")) return hit(mutableImage, context, match);
      return undefined;
    },
  },
  ...
  lineRule(insecureTls, /(?:\b(?:curl|wget)\b[^\n]*(?:\s-k(?:\s|$)|--insecure\b)|NODE_TLS_REJECT_UNAUTHORIZED\s*=\s*["']?0|GIT_SSL_NO_VERIFY\s*=\s*["']?(?:1|true))/i),
  {
    ...
    detect(context) {
      ...
      if (match === null) return undefined;
      const value = match[1] ?? "";
      if (/^(?:example|placeholder|changeme|redacted|dummy|test|your[_-])/i.test(value) || /^x+$/.test(value)) return undefined;
...
export function runRules(input: ScanInput, policy: Policy): RuleHit[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const context: LineContext = { input, lines, index, line, window, policy };
